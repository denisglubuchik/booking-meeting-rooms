...
class UploadOfficeImageUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        self.logger.debug(
            "upload_office_image_usecase_started office_id=%s content_type=%s size_bytes=%s",
            office_id,
            content_type,
            len(data),
        )
        if content_type not in _ALLOWED_IMAGE_CONTENT_TYPES:
            self.logger.warning(
                "upload_office_image_unsupported_content_type office_id=%s content_type=%s",
                office_id,
                content_type,
            )
            raise BadRequest("Unsupported image content type")
        if not data:
            self.logger.warning("upload_office_image_empty_payload office_id=%s", office_id)
            raise BadRequest("Image file is empty")

        async with self.office_repo:
            office = await self.office_repo.get_by_id(office_id)
            if office is None:
                self.logger.warning("upload_office_image_not_found office_id=%s", office_id)
            ...
            self.logger.debug("upload_office_image_usecase_finished office_id=%s", office_id)
...
class DeleteOfficeImageUseCase:
    ...
    async def execute(self, office_id: UUID) -> None:
        self.logger.info("delete_office_image_usecase_started office_id=%s", office_id)
        async with self.office_repo:
            office = await self.office_repo.get_by_id(office_id)
            if office is None:
                self.logger.warning("delete_office_image_not_found office_id=%s", office_id)
                raise NotFoundError(f"Office with id={office_id} not found")
            ...
            if office.image_key:
                ...
                await self.office_repo.save(office)
            self.logger.info("delete_office_image_usecase_finished office_id=%s", office_id)
