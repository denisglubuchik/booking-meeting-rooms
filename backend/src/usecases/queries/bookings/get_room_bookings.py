...
class GetRoomBookingsQueryHandler:
    def __init__(self, booking_repo: BookingsQueryInterface) -> None:
        self.booking_repo = booking_repo
        self.logger = logging.getLogger(
            "usecases.queries.bookings.get_room_bookings",
        )

    async def handle(
        self,
        query: GetRoomBookingsQuery,
    ) -> list[BookingResponseDTO]:
        self.logger.debug(
            "get_room_bookings_query_started room_id=%s",
            query.room_id,
        )
        ...
        self.logger.debug(
            "get_room_bookings_query_finished room_id=%s count=%s",
            query.room_id,
            len(bookings),
        )
