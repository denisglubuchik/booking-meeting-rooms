...
from usecases.exceptions import NotificationEnqueueError
    ...
    async def execute(
        ...
        try:
            ...
            self.logger.exception(
                "create_dispatch_failed user_id=%s type=%s channel=%s",
                dto.user_id,
                dto.notification_type,
                dto.channel,
            )
            raise NotificationEnqueueError(
                "Failed to enqueue notification dispatch",
            ) from exc
